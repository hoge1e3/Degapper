...
function ExpressionParser() {
	...
	function typeComposite() {
	    ...
	    $.reg=function (type, prio, a) {
	        ...
	        built.add(a.ret(pp.Parser.create(function (r) {
                r.opType=opt;
                return r;
	...
	$.mkInfix.def=function (left,op,right) {
		return pp.setRange({type:"infix", op:op, left: left, right: right});
	};
	$.mkInfixl=function (f) {
		$.mkInfixl.def=f;
	};
	$.mkInfixl.def=function (left, op , right) {
		return pp.setRange({type:"infixl",op:op ,left:left, right:right});
	};
	$.mkInfixr=function (f) {
		$.mkInfixr.def=f;
	};
	$.mkInfixr.def=function (left, op , right) {
		return pp.setRange({type:"infixr",op:op ,left:left, right:right});
	};
	$.mkPrefix=function (f) {
		$.mkPrefix.def=f;
	};
	...
	$.build= function () {
	    ...
	    $.built= pp.Parser.create(function (st) {
	        return parse(0,st);
	    }).setName("ExpBuilt");
	    return $.built;
	};
