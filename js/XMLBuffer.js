...
function XMLBuffer(src) {
	...
	$=function (node, attrName){
		...
		var hasTag=XMLBuffer.isValidTagName(node.type);
		if (hasTag) $.tag("<"+node.type+">"); // range=\""+(node.pos)+"-"+(node.len)+"\"
...
XMLBuffer.SUBELEMENTS="[XMLSUBELEMENTS]";
XMLBuffer.isValidTagName=function (t) {
    return typeof t=="string"
        && t.match(/^[a-zA-Z][0-9a-zA-Z_\-]*$/);
};