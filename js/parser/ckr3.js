...
CKR2=function () {
    ...
    function tk(r, name) {
        ...
        //pat.setName(name);
        var res=space.andNoUnify(pat).ret(function(a, b) {
            var res={};
            res.pos=b.pos;
            res.len=b.len;
            res.type=name;
    ...
    function commaSep(singular, plural ){
        ...
        return gr[plural]= G(singular).sep1(COM, true).ret(function (r) {
            ...
            if (r==null) throw this.name+" result is null";
    ...
    var reserved={
    "int":true , "double": true, "float": true, "char":true, "void":true,
                 "return":true,
                 "if":true,
                 "for":true,
                 "while":true,
                 ...
                 "static":true,
                 "extern":true, "typedef":true,"struct":true,
                 "sizeof": true,
    ...
    /*var type_symbol = typedef_name;  //identifier;
    ...
    var typedef_name=g("typedef_name", Parser.create(function (st) {
         var g=st.getGlobal();
         var res=identifier.parse(st);
         if (res.isSuccess() && g.structs[ res.result[0].text ]) {
             return res;
         }
         st.success=false;
         return st;
...
//        ands(sp("("),  abstract_declarator ,sp(")") ),
    ...
    var decl_tail = ors(
            ands( LB , constant_expression.or(identifier).opt() , RB),
    ...
    var struct_or_union=tk("struct").or(tk("union"));
    var struct_declaration_list=plural("struct_declaration");
    console.log("struct_or_uni");
    var struct_or_union_specifier=g("struct_or_union_specifier", ors(
            ands( struct_or_union, identifier.opt(), LC, struct_declaration_list ,RC),
            ands( struct_or_union, identifier)
    ));
    console.log("/struct_or_uni");
    var struct_declarator=g("struct_declarator", ors(
         declarator , ands(declarator.opt(), tk(":"), constant_expression )
    ));
    var struct_declarator_list=struct_declarator.sep1(COM,true);
    /*var struct_declaration=g("struct_declaration", ands(
         G("declaration_specifiers"), struct_declarator_list ,SC
    ));*/
    var struct_declaration=g("struct_declaration", ands(
    ...
    */
    var type_specifier=g("type_specifier",ors(
    	    tk("void"), tk("char"), tk("short"), tk("int"), tk("long"), tk("float"), tk("double"), tk("signed"),
            tk("unsigned"),
    ...
    // another version, differ from k&r:
    /*var declaration_specifiers=g("declaration_specifiers", ands(
            ors(storage_class_specifier, type_qualifier).rep0().ret(makeRepNodeF())
            , type_specifier));*/
    /*This allows:
          static cont my_type a;   // my_type is not typedefed But OK!
    */
    // k&r original  :
    var declaration_specifier=g("declaration_specifier", ors(
             storage_class_specifier,  // static  typedef
             type_specifier,  // int     struct{...}
             type_qualifier   // const / volatile
    )); // static / int/  const
    var declaration_specifiers=plural("declaration_specifier","declaration_specifiers");
    var declaration=g("declaration", ands(
        //   const       int           x =  5,y=4               ;
            declaration_specifiers, init_declarator_list.opt(), SC
    ).ret(Parser.create(function (st) {
         var node=st.result[0];
         var dsp=node["-element0"];
         var idl=node["-element1"];
         //console.log("declaration spec="+disp(dsp));
         if (dsp["-element0"] && dsp["-element0"].type=="typedef") {
	         if (idl.type=="IDENT") {
	             st.getGlobal().structs[idl.text]=true;
	             console.log("register struct ="+idl.text);
	         }
         }
         return st;
    })));
...
//  g\("(\w+)   var $1=g\("$1
    ...
    $.parse = function (str) {
        function preprocess(s) {
            ...
            s.split(/\n/).forEach(function (l) {
                if (l.match(/^\s*#(.*)/)) {
                	/*if (l.match(/^\s*#define\s+([a-zA-Z_0-9]+)\s+([a-zA-Z_0-9]+)/)) {
        ...
        str=preprocess(str);
        var res=sp.parse(translation_unit, str, {structs:{}});
