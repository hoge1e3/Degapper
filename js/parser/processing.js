...
Processing=function () {
	...
	$.parse = function (str) {
		...
		var newExpr = g("newExpr").ands(tk("new"), G("typeHead"), arrayElem.rep1() ).ret(null, "elementType","-subscript");
		...
		var curledExpr=g("curledExpr").ands(tk("{"), G("initializer").sep0(tk(",")) , tk("}") ).ret(null, "-value");
		var initializer=g("initializer").ors(expr, curledExpr);
		var varDeclElem=g("varDeclElem").ands(symbol, arrayDecl.rep0(),  tk("=").and(initializer).retN(1).opt() ).ret("name","arrays", "init");
		...
		var program=g("program").ands(decl.or(stmt).rep0() , space, sp.eof ).ret("-decls");
