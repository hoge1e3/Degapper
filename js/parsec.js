...
(function(){
    ...
    $.extend(P.prototype, {
        parse : function(s){ throw "abstract"; },// State->State

        and : function(q) {// Function|Parser  -> Parser
        	nc(q,"q");
        	var self = this;
            return P.create(function(s){
            	//nc(s,"s");
            	var m = self.parse(s);
            	//console.log("and 1 res="+disp(m.result));
                switch(m.type){
                case "fail"    : return m;
                case "success" :
                	...
                	//console.log("and 2 res="+disp(m.result));
                    var m2 = q.parse(m);
                	//console.log("and 3 res="+disp(m.result));
                    switch(m2.type) {
                    case "fail"    : return m2;
                    case "success" :
                    	//console.log("Concat "+disp(m.result)+" and "+disp(m2.result));
                        m2.result = m.result.concat(m2.result);
                    	//console.log("Concat res="+disp(m2.result));
                        return m2;
                    }
                }
            });
        ...
        or : function(q) {
        	...
        	return P.create(function(s){
        		//nc(s,"s");
        		//var ss=s.clone();
        		//if (s) console.log("or "+s.pos);
        		var m = self.parse(s);
        		switch(m.type){
        		case "fail" :
        			//if (s) console.log("or fail back to "+s.pos);
            		var m2 = q.parse(s);
            		//if (s) console.log("after back to "+s.pos+" res="+m2.type+" / "+m2.pos);
            		switch(m2.type) {
        			case "fail"    :
        				if(m2.pos < m2.pre.pos) return m;
        				else                    return m2;
    ...
    var ST = $.ParseState = function() {
        $.extend(this, {maxPos: {pos:0} ,pos :0,result:[], type:"success", pre:{}});
    };
    $.extend(ST.prototype, {
    ...
    var PS = $.Parsers = function(str) {
        ...
        self._reg=function (r) {
            return P.create(function(pre){
                ...
                var mr;
                var spc=(r+""==="/^(\\s*(\\/\\*([^\\/]|[^*]\\/|\\r|\\n)*\\*\\/)*(\\/\\/.*\\n)*)*/");
                if (!spc&&traceToken) console.log("pos="+self.state.pos+" r="+r+"  c="+c.substring(0,40));
                if(self.state.pos <= str.length && (mr=r.exec(c))) {
                	if (!spc&&traceToken) console.log("reg:succ");
                	...
                	if (!spc&&traceToken) console.log("reg:fail");
                    self.state.type="fail";
                    return self.state;
                }
            });
        };
        self._str=function (r) {
            return P.create(function(pre){
                if(!pre) pre = new ST;
                self.state = pre.clone();
                self.state.pre = pre;
                var c = str.substring(self.state.pos);
                if (traceToken) console.log("pos="+self.state.pos+" r="+r+"  c="+c);
                if(self.state.pos <= str.length && c.substring(0,r.length)===r ) {
                	if (traceToken) console.log("str:succ");
                	...
                	if (traceToken) console.log("str:fail");
                    self.state.type="fail";
                    return self.state;
                }
            });
