...
void rankin(int rank[], int score) {
    int i;
    //ランクインの判定
    ...
    for (i=0 ; i<RANKCOUNT ;i++) {
        ...
        drawNumber(rank[i], 250, i*20+100);
    }
...
int main (void) {
  ...
  while(1) {
    ...
    while(1) {
      clear();
      drawString("Score ",150, 50);
      drawNumber(score,200,50);
      drawString("High Score ",100, 80);
      drawNumber(rank[0],200,80);
      movePlayer(&player);
