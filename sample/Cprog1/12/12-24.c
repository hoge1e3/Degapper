...
void rankin(Rank rank[], int score) {
    ...
    while (1) {
	    ...
	    if (edit) {
	        rank[cr].name[c]=(blink?'_':' ');
	        blink=!blink;
	        for (ks[0]='A'; ks[0]<='Z'; ks[0]++) {
	            if (getkey(ks)==1) {
	                rank[cr].name[c]=ks[0];
	                c++;
	            }
	        }
	        if (getkey("Left")==1 && c>0) c--;
	        if (getkey("Return")==1 && c>0) {   
	              rank[cr].name[c]='\0';
	              writeRank(rank); edit=0;
	         }
	    } else {
