...
void moveBullets(Sikaku bullet[], Sikaku enemy[])  {
int i;
	    for (i=0 ; i<3 ; i++) {
	       draw(&bullet[i]);
	       move(&bullet[i]);
...
void initGame(Sikaku *player, Sikaku enemy[], Sikaku bullet[]) {
...
void mainLoop(Sikaku *player, Sikaku enemy[], Sikaku bullet[]) {
	  while(1) {
	    clear();
	    movePlayer(player);
   	    moveEnemies(enemy, player);
	    moveBullets(bullet,enemy);
...
int main (void) {
  ...
  Sikaku bullet[3];
  while(1) {
	  initGame(&player, enemy, bullet);
	  setColor(255,0,0);
	  mainLoop(&player, enemy, bullet);
  }
}
